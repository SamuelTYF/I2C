...
TP_EVENT *sTP_EVENT=nullptr;
TP_EVENT *Scan()
{
  TP_Scan(0);
  //Serial.println(sTP_DEV.chStatus);
  if(sTP_DEV.chStatus&TP_PRESS_DOWN)
  {
    if(sTP_EVENT==nullptr)sTP_EVENT=new TP_EVENT(sTP_Draw.Xpoint,sTP_Draw.Ypoint);
    else sTP_EVENT->Set(sTP_Draw.Xpoint,sTP_Draw.Ypoint);
  }
  else if(sTP_EVENT!=nullptr)
  {
    TP_EVENT *p=sTP_EVENT;
    sTP_EVENT=nullptr;
    return p;
  }
  return nullptr;
